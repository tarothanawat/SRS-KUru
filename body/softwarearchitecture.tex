...
\begin{itemize}[leftmargin=80pt]
    \item \textbf{Current POC baseline (May~2026):} The current RAG POC is evaluated with custom LLM-as-judge regression tests tracked in MLflow, including TCAS, fee, alias, bilingual, and missing-data honesty checks. The v8 structured RAG run records 72.7\% good answers on the fee/TCAS regression suite and a 74\% headline retrieval benchmark. This baseline is reported honestly as POC evidence, not as final MVP success.
