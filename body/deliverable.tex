...
The delivered software will include: a deployed web application accessible at a public URL; the data ingestion pipeline for มคอ.2 PDF processing and structured course data extraction; the Neo4j knowledge graph populated with all KU programs; the RAG pipeline and hybrid recommendation engine; and the ten core MVP features described in Chapter~\ref{chap:introduction}. The portfolio readiness checker (Feature~8) is scoped to the top 10 most popular KU programs. The recommendation engine (Feature~2) is initially scoped to returning the top 10 ranked programs, with the architecture designed for full coverage extension.
