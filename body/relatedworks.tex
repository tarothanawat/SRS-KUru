...
\begin{tabularx}{\textwidth}{|L{2.3cm}|L{1.4cm}|L{1.6cm}|L{1.9cm}|L{2.3cm}|Y|}
\hline
...
\hline
Career-to-PLO mapping & \xmark & \xmark & $\sim$ & $\sim$ & Phase~2 graph \\
\hline
TCAS admission guide & Scattered & Full --- all universities & $\sim$ & \xmark & Structured, KU only \\
\hline
Personalization & \xmark & \xmark & Session-only & Basic & RIASEC profile; progressive Phase~2 \\
\hline
...
KUru's interest discovery interface is grounded in Holland's RIASEC model (Realistic, Investigative, Artistic, Social, Enterprising, Conventional), the dominant framework in vocational psychology for connecting individual interests to occupational environments \cite{holland1997}. This theoretical foundation is directly compatible with KUru's O*NET career data layer \cite{onet2024}, which uses RIASEC codes to classify all 900+ occupations. Rather than presenting abstract RIASEC labels, KUru presents six sequential Likert screens using relatable Thai-language statements grounded in Holland's six dimensions, reducing cognitive load for a high school student audience while producing a more reliable initial vector than a tile-selection approach. The elicitation design is defined in Section~\ref{subsection:model-design}.
